...
# Get unique words
unique_words = melted_df['word'].unique()

# Create individual graphs for each word
for word in unique_words:
    # Filter data for the current word
    word_df = melted_df[melted_df['word'] == word]
    
    # Create the figure
    fig = px.line(word_df, x='year_month', y='rank', title=f'Popularity of "{word}" Over Time')
    ...
    fig.update_traces(
        ...
        meta=[word] * len(word_df)  # The meta parameter needs to be a list matched to the data points
    )
    
    # Save the figure as an SVG file
    fig.write_image(f"{word}_plot.svg")