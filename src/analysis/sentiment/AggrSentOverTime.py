...
def apply_sma(series, window=30):
...
def aggregate_sentiment_over_time(folder_path, trust_score_threshold):
    ...
    two_year_ago = pd.Timestamp.now(tz='UTC') - pd.DateOffset(years=2)
    ...
    df['sentiment_smoothed'] = apply_sma(df['sentiment'], window=30)
    ...
    sentiment_by_date['created_at'] = pd.to_datetime(sentiment_by_date['created_at'])

    # Set 'created_at' as the index
    sentiment_by_date.set_index('created_at', inplace=True)

    # Resample the data to weekly frequency
    sentiment_by_week = sentiment_by_date.resample('M').mean().reset_index()

    return sentiment_by_week
...
def plot_sentiment_over_time_plotly(df):
    ...
    # Add trace for trend line
    fig.add_trace(go.Scatter(x=df['created_at'], y=df['sentiment_smoothed_clipped'], mode='lines', name='Trend', line=dict(color='red', width=2)))
    ...
    fig.update_layout(
        ...
        yaxis=dict(showgrid=True, gridcolor='rgba(102, 102, 102, 0.2)', zeroline=False, range=[-0.2, 0.5]),
        autosize=False,
        margin=dict(autoexpand=False, l=100, r=20, t=110),
        showlegend=True,
        plot_bgcolor='white',
        # Adjust width and height
        width=1400,  # or any other value that fits your screen
        height=800  # or adjust according to your preference
    )
